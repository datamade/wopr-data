...
FROM '/project/evtimov/wopr/data/chicago-crimes-all_%s.csv'
WITH DELIMITER ','
CSV HEADER;
...
import os
srcFiles = [ f for f in os.listdir(SRC_DIR) if "chicago-crimes-all" in f ]
srcFiles.sort()
srcFiles = [ f for f in srcFiles if f[len("chicago-crimes-all_"):-4] >= START_DATE ]


SQL_DIR = "/project/evtimov/wopr/sql/"

fp = open(SQL_DIR + 'update/'+ "updates_chicago-crimes-all.sql", 'w')
...
for srcF in srcFiles[:2]:
    dateStr = srcF[len("chicago-crimes-all_"):-4]
